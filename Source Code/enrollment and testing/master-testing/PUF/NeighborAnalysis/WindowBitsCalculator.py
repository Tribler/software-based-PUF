...
class WindowBitsCalculator:
    ...
    def initialize_bits_score(self, bits, length=262144):
        """
        Initialize the score of every bit. A stable bit is given 1 and unstable is 0
        :param bits: Locations of stable bits
        :param length: total length of bits in an SRAM. 262144 total bits in Cypress CY62256NLL and
                1048576 is total bits in Microchip 23LC1024
        """
    ...
    def calculate(self, window=8, max_length=1048576):
        """
        Calculate the rank of btis using neighbor analysis algorithm
        :param window:
        :param max_length: total length of bits in an SRAM. 262144 total bits in Cypress CY62256NLL and
                1048576 is total bits in Microchip 23LC1024
        """
        for i in self.score:
            if self.score[i] > 0:
