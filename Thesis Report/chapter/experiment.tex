...
\section{Testing on Selected SRAMs}
...
\section{Experiments on Bit Selection Algorithms}
...
\section{Testing on A Set of Bit Locations as A Challenge}
...
\section{Complete Enrollment Scheme}
...
% \begin{table}[htbp]
%   \centering
%   \caption{Time measurement of the secure data protection and key storage scheme in \si{\micro}s.}
%     \begin{tabular}{|c|c|c|c|c|c|c|}
%     \hline
%     \textbf{No} & \textbf{Stage 1} & \textbf{Stage 2} & \textbf{Stage 3} & \textbf{Stage 4} & \textbf{Stage 5} & \textbf{Stage 6} \\
%     \hline
...
% Figure \ref{fig:decryption_correct_pass} displays the decryption of bitcoin key is successful when the user's password and the SRAM is correct. Meanwhile, Figure \ref{fig:decryption_wrong_pass} present the failure of bitcoin key reconstruction when an incorrect user's password is utilized.
