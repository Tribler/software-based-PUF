...
\textit{This chapter contains our proposed system to achieved our goals which explained in Chapter 1. First, use cases, assumptions and requirements on our system are presented. The chapter continues with reasonings on our chosen embedded platform, Arduino. Then, the selected error correcting code used in the system is explained. Afterwards, we present our data protection and key storage scheme and also our way to generate key using SRAM PUF. Last, our idea to use bits locations as a PUF challenge is shown. }

\section{Use Case, Assumptions and Requirements}\label{ch:use_case}
...
Due to the above consideration, Arduino is chosen. Even though one has to construct the system in C++, this can be a positive thing since one can maximize the computing capability easily.
...
\section{Data Protection and Key Storage Scheme} \label{chp:data_protection_scheme}
